...
public class User {
    ...
    public boolean reg() throws Exception {
        ...
        Message.sendMessage(this, "Введите имя:",3); // Отправляем сообщение пользователю
        String name = Message.readMessage(this).getMsg(); // Читаем ответ пользователя
        Message.sendMessage(this, "Введите логин:",3); // Отправляем сообщение пользователю
        String login = Message.readMessage(this).getMsg(); // Читаем ответ пользователя
        Message.sendMessage(this, "Введите пароль:",3); // Отправляем сообщение пользователю
        ...
        if (resultSet.next()){ // Если next() вернёт true, значит пользователь уже есть
            Message.sendMessage(this, "Такой пользователь уже зарегистрирован",3); // Отправляем сообщение пользователю
            return false;
    ...
    public boolean login() throws Exception{
        ...
        if(reresultSet.next()){ // Если такой пользователь есть
            ...
            Message.sendMessage(this, "Неверный логин или пароль",3); // Отправляем сообщение пользователю
