                    ...
                    public void run() {
                        try {
                            Message.sendMessage(user, "Введите команду /reg - для регистрации\n" +
                                    "/login - для авторизации",3); // Отправляем jsonObject пользователю user
                            while (true){
                                ...
                                if(command.equals("/reg")){
                                    ...
                                    Message.sendMessage(user, "Неверная команда, попробуйте ещё раз",3);
                                }
                            }

                            Message.sendMessage(user, user.getName()+" добро пожаловать на сервер",3); // Отправляем сообщение пользователю
